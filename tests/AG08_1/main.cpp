...
Utils utils;
Camera camera(glm::vec3(-1.0f, 2.0f, 6.0f)); //FIX
...
const char* pathProyecto = "../tests/AG08_1/";
...
void Render(uint32_t VAO, const Shader& shaderCube, const Shader& shaderlight,
	...
	shaderCube.Set("shininess",1);
	shaderCube.Set("specularStrenght", 0.3f);
	glBindVertexArray(VAO);
...
uint32_t createVertexData(const float* vertices, const uint32_t n_verts, const uint32_t* indices, const uint32_t n_indices) {
	...
	////Vertices de textura
	//glVertexAttribPointer(atributteNumber, 2, GL_FLOAT, GL_FALSE, _numberOfElementsPerLine * sizeof(float), (void*)(stride * sizeof(float)));
	//glEnableVertexAttribArray(1);
	//stride += 2;

	//Vertices normal
	glVertexAttribPointer(atributteNumber, 3, GL_FLOAT, GL_FALSE, _numberOfElementsPerLine * sizeof(float), (void*)(stride * sizeof(float)));
	glEnableVertexAttribArray(atributteNumber);
